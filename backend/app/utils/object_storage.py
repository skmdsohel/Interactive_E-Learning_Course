...
Three backends are supported, selected by `STORAGE_BACKEND`:
...
Seeded-on-disk content keeps working under the cloud backends: when a key is
...
class _AzureBlobBackend:
    ...
    def _object_key(self, key: str) -> str:
        # Same layout as the local + R2 backends so DB rows stay portable:
...
_backend: _LocalBackend | _R2Backend | _AzureBlobBackend | None = None


def get_storage_backend() -> _LocalBackend | _R2Backend | _AzureBlobBackend:
    ...
    if kind == "r2":
        ...
        raise RuntimeError(
            ...
            "Expected one of: local, r2, azure."
        )
    return _backend
